...
import { assertFirstMintReady } from './Dominum/foundation/mintPreflight';
...
async function main(): Promise<void> {
  ...
  const controls = loadDomRecipientControls();
  ...
  const infrastructure: InfrastructureContracts = {
    ...
    gasPool: provider.open(
      GasPool.createFromAddress(addresses.gasPool)
    ),
  };
  ...
  provider.ui().write('DOM FIRST MINT PREFLIGHT: OK');
  provider.ui().write('No mint transaction was submitted.');
}
