...
const GIVER_DAO = 'Dominum/givers/GiverDao' as const;
...
describe('GiverDao', () => {
  ...
  let newBankDao: SandboxContract<TreasuryContract>;
  let newDaoFoundation: SandboxContract<TreasuryContract>;
  ...
  beforeAll(async () => {
    ...
    giverCode = await compile(GIVER_DAO);
  });
  ...
  beforeEach(async () => {
    ...
    newBankDao = await blockchain.treasury('new-bank-dao');
    newDaoFoundation = await blockchain.treasury('new-dao-foundation');
  ...
  it('should update wallet and whitelist only from manager', async () => {
    ...
    await ignoreFailure(
      giver.sendChangeWhitelist(
        ...
        {
          value: toNano('0.05'),
          bankDaoAddress: newBankDao.address,
          daoFoundationAddress: newDaoFoundation.address,
          queryId: 7n,
        }
      )
    );
    ...
    expect(giverData.bankDaoAddress.toString()).toEqual(bankDao.address.toString());
    expect(giverData.daoFoundationAddress.toString()).toEqual(daoFoundation.address.toString());
    ...
    expect(giverData.bankDaoAddress.toString()).toEqual(newBankDao.address.toString());
    expect(giverData.daoFoundationAddress.toString()).toEqual(newDaoFoundation.address.toString());
  });

  it('should auto-distribute minted DOM between DAO bank and DAO foundation', async () => {
    const {
      domMaster,
      gasPool,
      giver,
    ...
    const bankWallet = blockchain.openContract(
      DomWallet.createFromAddress(
        await domMaster.getWalletAddress(bankDao.address)
      )
    );
    const foundationWallet = blockchain.openContract(
      DomWallet.createFromAddress(
        await domMaster.getWalletAddress(daoFoundation.address)
      )
    );
