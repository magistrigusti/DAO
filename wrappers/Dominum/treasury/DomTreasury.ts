...
export function domTreasuryConfigToCell(config: DomTreasuryConfig): Cell {
  return beginCell()
...
export class DomTreasury implements Contract {
  ...
  static createFromConfig(config: DomTreasuryConfig, code: Cell, workchain = 0) {
    const data = domTreasuryConfigToCell(config);
    const init = { code, data };
    return new DomTreasury(contractAddress(workchain, init), init);
  }
  ...
  async getTreasuryData(provider: ContractProvider) {
    const { stack } = await provider.get('getTreasuryData', []);
